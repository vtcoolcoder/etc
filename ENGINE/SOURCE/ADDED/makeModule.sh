...
isChangeFontColor()    # 2
{
    showTracingMessage "isChangeFontColor( \
       \
        \$isEnableColors, \$currentFontColor )"
...
{   
    ...
    showTracingMessage "isFileMatchToRegExp( \
       \
        \$fmtRegExpr, \$basenameSourceFile )"
...
{     
    ...
    showTracingMessage "showServiceMsg( \
       \
        \$currentFontColor, \$currentServiceMessage )"
...
if [[ -e "$HOME/bin/ENGINE/SOURCE/CORE.SH" ]]; then

    source "$HOME/bin/ENGINE/SOURCE/CORE.SH"
    
else

    echo "Импортируемый файл 'CORE.SH' не найден!" >&2
    exit 1
...
executeMode="$ADDPREFIX"

# Флаг отображения доп. сообщений о состоянии
isEnableVerbose="$ON"

# Флаг вкл/откл цветной подсветки сообщений
isEnableColors="$ON"
...
# ******************************************************************************
#
#                     УДАЛЯЕТ ПРЕФИКС ИЗ ИМЕНИ ФАЙЛА
#
# ******************************************************************************
#                                ИМПОРТ ДВИЖКА
# ******************************************************************************
...
# Режим работы: не изменять!
executeMode="$CUTSUFFIX"

# Значение по умолчанию удаляемого суффикса
defaultAffixValue=".*"
...
# ******************************************************************************
#
#              НОРМАЛИЗУЕТ ВСЕ БУКВЫ ИМЕНИ ФАЙЛА В СТРОЧНЫЕ
#
# ******************************************************************************
#                                ИМПОРТ ДВИЖКА
# ******************************************************************************
...
# ******************************************************************************
#
#     ДЕЛАЕТ СТРОЧНЫМ ПЕРВЫЙ СИМВОЛ ИМЕНИ ФАЙЛА, ЕСЛИ ОН ЯВЛЯЕТСЯ БУКВОЙ
#
# ******************************************************************************
#                                ИМПОРТ ДВИЖКА
# ******************************************************************************
...
# ******************************************************************************
#
#     ДЕЛАЕТ ПРОПИСНЫМ ПЕРВЫЙ СИМВОЛ ИМЕНИ ФАЙЛА, ЕСЛИ ОН ЯВЛЯЕТСЯ БУКВОЙ
#
# ******************************************************************************
#                                ИМПОРТ ДВИЖКА
# ******************************************************************************
...
# ******************************************************************************
#
#             НОРМАЛИЗУЕТ ВСЕ БУКВЫ ИМЕНИ ФАЙЛА В ПРОПИСНЫЕ
#
# ******************************************************************************
#                                ИМПОРТ ДВИЖКА
# ******************************************************************************
...
makeContent()
{
    if [[ ! -d "CONTENT" ]]
    then
        if mkdir -p "CONTENT/SOURCE/ATTRIBUTES" "CONTENT/SOURCE/FUNCS"
        then
    
            makeModuleConsts | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/ATTRIBUTES/CONSTS.SH"
            makeModuleVars | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/ATTRIBUTES/VARS.SH"
    
            makeModuleCheckings | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/CHECKINGS.SH"
            makeModuleDefings | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/DEFINGS.SH"
            makeModuleEtc | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/ETC.SH"
            makeModuleMainings | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/MAININGS.SH"
            makeModulePrintings | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/PRINTINGS.SH"
            makeModuleSettings | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/SETTINGS.SH"
            makeModuleShowings | sed 's/\\\$/\$/g' > "CONTENT/SOURCE/FUNCS/SHOWINGS.SH"
            
            makeModuleCore > "CONTENT/SOURCE/CORE.SH"
            
            makeModuleAddprefix > "CONTENT/SOURCE/addprefix"
            makeModuleAddsuffix > "CONTENT/SOURCE/addsuffix"
            makeModuleCutprefix > "CONTENT/SOURCE/cutprefix"
            makeModuleCutsuffix > "CONTENT/SOURCE/cutsuffix"
            makeModuleLower > "CONTENT/SOURCE/lower"
            makeModuleLowfirst > "CONTENT/SOURCE/lowfirst"
            makeModuleUpfirst > "CONTENT/SOURCE/upfirst"
            makeModuleUpper > "CONTENT/SOURCE/upper"
        fi
    fi
}
...
createSymbolicLinks()
{
    if [[ "$PWD" != "CONTENT" ]] && cd "CONTENT"
    then
    
        ln -s "SOURCE/CORE.SH" "core"
         
        declare -a args=("SOURCE/addprefix")
        args+=("SOURCE/addsuffix")
        args+=("SOURCE/cutprefix")
        args+=("SOURCE/cutsuffix")
        args+=("SOURCE/lower")
        args+=("SOURCE/lowfirst")
        args+=("SOURCE/upfirst")
        args+=("SOURCE/upper")
