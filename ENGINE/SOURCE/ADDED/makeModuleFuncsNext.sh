...
declare -g isIncludeTracing="$OFF"


includeTracing()
{
    (( isIncludeTracing == "$ON" )) && echo "$1"
}
...
$(includeTracing 'declare -g -r TRACING_FONT_COLOR="\033[1;35m"    # сиреневый')
...
$(includeTracing 'declare -g -r PFORMAT_TRACING="%b\tТрассировка: %s\t\t%b\n"')
...
$(includeTracing '# флага вкл/откл трассировки
...
isHideMessage()    # 1
{
    $(includeTracing 'showTracingMessage "isHideMessage( \$isEnableVerbose, \$renameStatus )"')
...
isChangeFontColor()    # 2
{
    $(includeTracing 'showTracingMessage "isChangeFontColor( \$isEnableColors, \$currentFontColor )"')
...
{   
    ...
    $(includeTracing 'showTracingMessage "isFileMatchToRegExp( \$fmtRegExpr, \$basenameSourceFile )"')
...
{   
    ...
    $(includeTracing 'showTracingMessage "isFileRenamed( \$sourceFile, \$destinationFile )"')
...
isSetSupportMode()    # 5
{
    $(includeTracing 'showTracingMessage "isSetSupportMode( \$executeMode )"')
...
isSetArgs()    # 6
{
    $(includeTracing 'showTracingMessage "isSetArgs( \$TOTAL_FILES )"')
...
{        
    ...
    $(includeTracing 'showTracingMessage "isSetValue( \$currentAffix )"')
...
{   
    $(includeTracing 'showTracingMessage "isPreparelyChecked()"')
...
isFileExist()    # 9
{   
    $(includeTracing 'showTracingMessage "isFileExist( \$sourceFile )"')
...
isFileDirectory()    # 10
{   
    $(includeTracing 'showTracingMessage "isFileDirectory( \$sourceFile )"')
...
isFileChecked()    # 11
{   
    $(includeTracing 'showTracingMessage "isFileChecked( \$sourceFile )"')
...
{   
    ...
    $(includeTracing 'showTracingMessage "defPrintMessageAttr( \$renamingMarker )"')
...
{  
    $(includeTracing 'showTracingMessage "defResultDestination( \$isEnableCaching )"')
...
{
    $(includeTracing 'showTracingMessage "defFileCheckedFinalStep( \$executeMode )"')
...
{ 
    $(includeTracing 'showTracingMessage "defPrepareCheckedFinalStep( \$executeMode )"')
...
defRegExpr()    # 5
{     
    $(includeTracing 'showTracingMessage "defRegExpr( \$executeMode )"')
...
{   
    $(includeTracing 'showTracingMessage "defFileDestination( \$sourceFile )"')
...
{
    $(includeTracing 'showTracingMessage "defCurrentContainer()"')
...
{   
    ...
    $(includeTracing 'showTracingMessage "makeWarning( \$tmpMessage )"')
...
{   
    $(includeTracing 'showTracingMessage "encodingMetaSymbols()"')
...
cacheRenameState()    # 3
{   
    $(includeTracing 'showTracingMessage "cacheRenameState( \$renameStatus )"')
...
tryRenameFile()    # 4
{ 
    $(includeTracing 'showTracingMessage "tryRenameFile( \$sourceFile )"')
...
parseArgs()    # 5
{   
    $(includeTracing 'showTracingMessage "parseArgs( \${ARGV[@]} )"')
...
main()    # 1
{   
    $(includeTracing 'showTracingMessage "main()"')
...
resetParams()    # 4
{
    $(includeTracing 'showTracingMessage "resetParams()"')
...
updateArgv()    # 5
{
    $(includeTracing 'showTracingMessage "updateArgv()"')
...
printGeneralState()    # 1
{
    $(includeTracing 'showTracingMessage "printGeneralState()"')
...
{
    $(includeTracing 'showTracingMessage "printCachedWarnings()"')
...
{
    $(includeTracing 'showTracingMessage "printCachedRenames()"')
...
printRenameState()    # 4
{    
    $(includeTracing 'showTracingMessage "printRenameState( \$renameStatus )"')
...
{
    $(includeTracing 'showTracingMessage "printSortedResults()"')
...
printCachedResults()
{
    $(includeTracing 'showTracingMessage "printCachedResults()"')
...
setMsgFontColor()    # 1
{
    $(includeTracing 'showTracingMessage "setMsgFontColor()"')
...
setValue()    # 2
{
    $(includeTracing 'showTracingMessage "setValue()"')
...
setValidValue()    # 3
{
    $(includeTracing 'showTracingMessage "setValidValue()"')
...
{   
    ...
    $(includeTracing 'showTracingMessage "showCachedRenames( \$currentRenameType )"')
...
showCachedResults()    # 2
{
    $(includeTracing 'showTracingMessage "showCachedResults()"')
...
{     
    ...
    $(includeTracing 'showTracingMessage "showServiceMsg( \$currentFontColor, \$currentServiceMessage )"')
...
showVerbose()    # 4
{  
    $(includeTracing 'showTracingMessage "showVerbose()"')
...
$(includeTracing 'showTracingMessage()    # 5
{                      
...
$(includeTracing 'showParentName()    # 6
{ 
    if (( isEnableTracing == "\$ON" ))
...
showMenu()    # 7
{   
    $(includeTracing 'showTracingMessage "showMenu()"')
...
makeContent()
{
    ...
    then
        ...
        then
            ...
            makeModuleConsts | sed 's/\\\$/\$/g' > "CONTENT-mid/ATTRIBUTES/CONSTS-mid.SH"
            makeModuleVars | sed 's/\\\$/\$/g' > "CONTENT-mid/ATTRIBUTES/VARS-mid.SH"
    
            makeModuleCheckings | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/CHECKINGS-mid.SH"
            makeModuleDefings | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/DEFINGS-mid.SH"
            makeModuleEtc | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/ETC-mid.SH"
            makeModuleMainings | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/MAININGS-mid.SH"
            makeModulePrintings | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/PRINTINGS-mid.SH"
            makeModuleSettings | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/SETTINGS-mid.SH"
            makeModuleShowings | sed 's/\\\$/\$/g' > "CONTENT-mid/FUNCS/SHOWINGS-mid.SH"
        fi
    fi
}
