...
public final class DAO {
    ...
    private static final String SELECT_ID_AND_NOTE_FRAGMENT = """
        SELECT id, SUBSTRING(note, 0, 230) AS fragment
        FROM java
        WHERE subject = ?
        ORDER BY timestamp DESC
        """;
        
    private static final String SELECT_NOTE_FRAGMENT_TO_ID = """
        SELECT id, SUBSTRING(note, 0, 230) AS fragment
        FROM java
        ORDER BY LOWER(subject), note, timestamp DESC
        """;
    ...
    public Map<Integer, String> getNoteFragmentToId() {
        /*
        @Getter
        ...
        */
        ...
        return jdbcTemplate.queryForStream(SELECT_NOTE_FRAGMENT_TO_ID,
                (rs, i) -> Map.entry(rs.getInt("id"), rs.getString("fragment")))
                .collect(toMap(Map.Entry::getKey, Map.Entry::getValue, 
                        (k, v) -> v, LinkedHashMap::new));
    }
    
    
    public Map<Integer, String> getIdAndNoteFragmentBySubject(String subject) {
        /*
        @Getter
        @RequiredArgsConstructor
        class Helper {
        ...
        */
        ...
        return jdbcTemplate.queryForStream(SELECT_ID_AND_NOTE_FRAGMENT,
                (rs, i) -> Map.entry(rs.getInt("id"), rs.getString("fragment")),
                subject)
                .collect(toMap(Map.Entry::getKey, Map.Entry::getValue, 
                        (k, v) -> v, LinkedHashMap::new));
    ...
    public Note getRandomNote() {             
        List<Integer> allId = List.copyOf(getAllId());   
        
        return jdbcTemplate.queryForStream(queries.getRandom(),   
                (rs, i) -> new Note(rs.getString("subject"), rs.getString("note")),
                allId.get(RANDOM.nextInt(allId.size())))
    ...
    public Set<Integer> getAllId() {
        return jdbcTemplate.queryForStream(queries.getAllId(), 
