...
import static test_lombok.Params.*;
...
public class TestLombok {
    ...
    public static void main(String[] args) {
        @Cleanup
        AutoCloseable autoCloseable = () -> System.out.println("Закрытие ресурса...");
        ...
        var params = builder().name("Goy Gaya").symbol('G').id(23).delta(23.23).on(true).build();
        showData(params);
    }
    ...
    private static void showData(Params params) {
        ...
        System.out.printf("Name: %s%nSymbol: %c%nId: %d%nDelta: %.2f%nIsOn: %b%n%n", 
                name, symbol, id, delta, isOn);
    }
    
    
    private static void showComparingInfo() {
